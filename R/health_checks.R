...
check_filesystem_health <- function() {
  ...
  if (Sys.info()["sysname"] != "Windows") {
    ...
    if (!is.null(disk_info) && length(disk_info) > 1) {
      # Parse disk usage
      disk_parts <- strsplit(disk_info[2], "\\s+")[[1]]
      if (length(disk_parts) >= 5) {
        usage_percent <- as.numeric(gsub("%", "", disk_parts[5]))
        metrics$disk_usage_percent <- usage_percent
        
        if (usage_percent > .health_config$disk_threshold_percent) {
          issues <- c(issues, sprintf("Disk usage high: %.0f%%", usage_percent))
        }
      }
    }
  }
  ...
  if (length(issues) == 0) {
    ...
    result$message <- "All file systems accessible"
...
check_memory_health <- function() {
  ...
  used_mb <- sum(mem_info[, "used"]) / (1024^2) * 8  # Convert to MB (assuming 8 bytes per unit)
  max_mb <- sum(mem_info[, "max used"]) / (1024^2) * 8
  
  result$metrics <- list(
    used_mb = round(used_mb, 2),
    max_used_mb = round(max_mb, 2),
    ncells = mem_info["Ncells", "used"],
    vcells = mem_info["Vcells", "used"]
  )
  
  # Check against threshold
  if (used_mb < .health_config$memory_threshold_mb) {
    result$status <- "healthy"
    result$message <- sprintf("Memory usage: %.0f MB", used_mb)
  } else if (used_mb < .health_config$memory_threshold_mb * 1.5) {
    result$status <- "warning"
    result$message <- sprintf("Memory usage elevated: %.0f MB", used_mb)
  } else {
    result$status <- "critical"
    result$message <- sprintf("Memory usage critical: %.0f MB", used_mb)
...
check_api_health <- function() {
  ...
  api_port <- Sys.getenv("API_PORT", "8080")
  api_url <- sprintf("http://localhost:%s", api_port)
  ...
  tryCatch({
    ...
    if (httr::status_code(response) == 200) {
      result$status <- if (response_time < 100) "healthy" else "warning"
    ...
    result$status <- "warning"  # Not critical since API might not be running
    result$message <- "API not reachable"
  })
...
check_auth_health <- function() {
  ...
  if (!file.exists("R/auth.R")) {
    result$status <- "critical"
    result$message <- "Authentication module missing"
    return(result)
  }
  ...
  if (!file.exists(users_file)) {
    result$status <- "critical"
    result$message <- "Users configuration missing"
    return(result)
  }
  ...
  tryCatch({
    ...
    if (has_plain_passwords) {
      result$status <- "warning"
      result$message <- "Plain text passwords detected - run auth_upgrade_hashes()"
    } else if (!has_admin) {
      result$status <- "warning"
...
check_audit_health <- function() {
  ...
  tryCatch({
    # Check file size
    size_mb <- file.info(audit_file)$size / 1024^2
    
    # Try to read last entries
    audit_data <- read.csv(audit_file, stringsAsFactors = FALSE)
    entry_count <- nrow(audit_data)
    
    # Check hash chain if available
    if ("hash" %in% names(audit_data) && "prev_hash" %in% names(audit_data)) {
      # Verify last few entries
      if (entry_count > 1) {
        last_entries <- tail(audit_data, 10)
        hash_valid <- TRUE
        
        for (i in 2:nrow(last_entries)) {
          if (last_entries$prev_hash[i] != last_entries$hash[i-1]) {
            hash_valid <- FALSE
            break
          }
        }
        ...
        if (!hash_valid) {
          result$status <- "critical"
          result$message <- "Audit hash chain broken"
          result$metrics <- list(
            entries = entry_count,
    ...
    result$status <- if (size_mb < 100) "healthy" else "warning"
    result$message <- sprintf("Audit log: %d entries (%.1f MB)", entry_count, size_mb)
    result$metrics <- list(
      entries = entry_count,
      size_mb = round(size_mb, 2),
...
check_model_health <- function() {
  ...
  required_models <- c(
    "stan" = "models/stan/pk_multicpt_ode.stan",
    "jags_1c" = "models/jags/pk_onecpt_inf.jags"
  )
  
  missing <- character()
  
  for (name in names(required_models)) {
    if (!file.exists(required_models[[name]])) {
